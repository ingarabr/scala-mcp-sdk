...
class UriTemplateSuite extends FunSuite {
  ...
  test("expand simple variable") {
    ...
    assertEquals(template.expand(Map("var" -> "value")), "value")
  }

  test("expand with literal prefix") {
    val template = UriTemplate.parse("file:///{path}").toOption.get
    assertEquals(template.expand(Map("path" -> "config.json")), "file:///config.json")
  }
  ...
  test("expand multiple variables") {
    ...
    assertEquals(template.expand(vars), "https://example.com/api")
  }

  test("expand undefined variable is omitted") {
    val template = UriTemplate.parse("{var}").toOption.get
    assertEquals(template.expand(Map.empty), "")
  }

  test("expand partial variables defined") {
    val template = UriTemplate.parse("{a}/{b}").toOption.get
    assertEquals(template.expand(Map("a" -> "first")), "first/")
  }

  test("expand encodes special characters") {
    val template = UriTemplate.parse("{var}").toOption.get
    assertEquals(template.expand(Map("var" -> "hello world")), "hello%20world")
  }

  test("expand encodes unicode") {
    val template = UriTemplate.parse("{var}").toOption.get
    assertEquals(template.expand(Map("var" -> "café")), "caf%C3%A9")
  }

  test("expand preserves unreserved characters") {
    val template = UriTemplate.parse("{var}").toOption.get
    assertEquals(template.expand(Map("var" -> "a-b_c.d~e")), "a-b_c.d~e")
  }

  test("expand comma-separated values") {
    val template = UriTemplate.parse("{a,b}").toOption.get
    assertEquals(template.expand(Map("a" -> "1", "b" -> "2")), "1,2")
  }
  ...
  test("extract simple variable") {
    val template = UriTemplate.parse("{var}").toOption.get
    assertEquals(template.extract("value"), Some(Map("var" -> "value")))
  }

  test("extract with literal prefix") {
    val template = UriTemplate.parse("file:///{path}").toOption.get
    assertEquals(template.extract("file:///config.json"), Some(Map("path" -> "config.json")))
  }

  test("extract multiple variables") {
    val template = UriTemplate.parse("user:///{id}/profile").toOption.get
    assertEquals(template.extract("user:///123/profile"), Some(Map("id" -> "123")))
  }

  test("extract returns None on non-match") {
    val template = UriTemplate.parse("file:///{path}").toOption.get
    assertEquals(template.extract("http:///foo"), None)
  }

  test("extract decodes percent-encoded values") {
    val template = UriTemplate.parse("{var}").toOption.get
    assertEquals(template.extract("hello%20world"), Some(Map("var" -> "hello world")))
  }

  test("extract empty variable value") {
    val template = UriTemplate.parse("prefix/{var}/suffix").toOption.get
    assertEquals(template.extract("prefix//suffix"), Some(Map("var" -> "")))
  }
  ...
  test("file template with encoded slashes") {
    ...
    assertEquals(template.expand(Map("path" -> "home/user/config.json")), "file:///home%2Fuser%2Fconfig.json")

    // Extract with encoded slashes
    assertEquals(template.extract("file:///home%2Fuser%2Fconfig.json"), Some(Map("path" -> "home/user/config.json")))
  }

  test("config template pattern") {
    val template = UriTemplate.parse("config:///{section}").toOption.get

    assertEquals(template.expand(Map("section" -> "database")), "config:///database")
    assertEquals(template.extract("config:///database"), Some(Map("section" -> "database")))
  }

  test("user profile template pattern") {
    val template = UriTemplate.parse("user:///{id}/profile").toOption.get

    assertEquals(template.expand(Map("id" -> "123")), "user:///123/profile")
    assertEquals(template.extract("user:///123/profile"), Some(Map("id" -> "123")))
  }
  ...
  test("roundtrip: expand then extract") {
    ...
    val extracted = template.extract(expanded)
